import{c as x,p as g,f as m,d as p,s as u,g as y,i as b,b as v}from"./breadcrumb-BskWwaws.js";import{c as h}from"./formatter-byuH9jKl.js";const f="/qveeco-ecommerce/assets/images",$=p();function r(){let t=0,a="";const n=x($);Object.values(n).forEach(e=>{const o=e.productId,s=g.find(c=>c.id===o);if(s){const c=s.priceCents*e.quantity;t+=c,a+=`
  ...
  `;document.getElementById("paymentSummaryContainer").innerHTML=d}const k="/qveeco-ecommerce/assets/images",i=p();function l(){let t="";i.forEach(n=>{const d=n.productId,e=g.find(o=>o.id===d);if(!e){console.warn("Product not found for ID:",d);return}t+=`
      ...
      <div class="h-full w-full space-y-1 md:space-y-4">
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <p class="ff-primary text-normal text-sm text-black md:text-base lg:text-lg">
              Size:&nbsp;<span class="text-normal text-black/60">${h(n.size)}</span>
            </p>
            <p class="ff-primary text-normal text-sm text-black md:text-base lg:text-lg">
              Quantity:&nbsp;<span class="text-normal text-black/60">${n.quantity}</span>
            </p>
          </div>
          <button class="delete-btn ml-auto flex-shrink-0" data-product-id="${n.productId}">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center rounded-full bg-gray-100 px-2 py-1">
            <button class="minus-btn px-2 text-xl font-medium text-black" data-product-id="${n.productId}"
            data-product-size="${n.size}">−</button>
  ...
  `});const a=document.getElementById("orderSummaryContainer");if(i.length===0){a.innerHTML=`
      <p class="text-center text-black/60 text-lg">Your cart is empty.</p>
    `;return}a.innerHTML=t}document.getElementById("orderSummaryContainer").addEventListener("click",t=>{const a=t.target.closest(".minus-btn"),n=t.target.closest(".add-btn"),d=t.target.closest(".delete-btn");if(a){const e=a.dataset.productId,o=a.dataset.productSize,s=i.find(c=>c.productId===e&&c.size===o);s&&s.quantity>1&&(s.quantity-=1,u(),l(),r(),console.log(s.quantity))}if(n){const e=n.dataset.productId,o=n.dataset.productSize,s=i.find(c=>c.productId===e&&c.size===o);s&&(s.quantity+=1,u(),l(),r(),console.log(s.quantity))}if(d){const e=d.dataset.productId,o=i.findIndex(s=>s.productId===e);o!==-1&&(i.splice(o,1),u(),l(),r(),L()),console.log(e)}});function L(){const t=document.getElementById("toast-remove");t&&(t.classList.remove("hidden"),setTimeout(()=>{t.classList.add("hidden")},3e3))}document.querySelector('[data-dismiss-target="#toast-remove"]')?.addEventListener("click",()=>{document.getElementById("toast-remove")?.classList.add("hidden")});document.addEventListener("DOMContentLoaded",()=>{JSON.parse(localStorage.getItem("cart"));const t=y(),a=t===0?13:t;document.getElementById("cartCount").textContent=a,b(),v(".breadcrumbs ul",[{label:"Home",href:"qveeco.html"},{label:"Products",href:"category.html"},{label:"Cart",href:"cart.html"}]),l(),r()});document.addEventListener("DOMContentLoaded",()=>{const t=p();document.querySelector(".checkout-btn")?.addEventListener("click",()=>{t.length===0?w():(console.log("Proceed to checkout"),window.location.href="checkout.html")})});function w(){const t=document.getElementById("toast-warning");t&&(t.classList.remove("hidden"),setTimeout(()=>{t.classList.add("hidden")},3e3))}document.querySelector('[data-dismiss-target="#toast-warning"]')?.addEventListener("click",()=>{document.getElementById("toast-warning")?.classList.add("hidden")});
